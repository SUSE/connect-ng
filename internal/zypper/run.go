...
func zypperRun(args []string, validExitCodes []int) ([]byte, error) {
	...
	if config.CFG.FsRoot != "" {
		cmd = append(cmd, "--root", config.CFG.FsRoot)
	}
	cmd = append(cmd, args...)
    logging.QuietOut.Printf("\nExecuting '%s'\n\n", strings.Join(cmd, " "))
	output, err := utils.Execute(cmd, validExitCodes)
	if err != nil {
		if ee, ok := err.(utils.ExecuteError); ok {
			return nil, utils.ZypperError(ee)
		}
	}
	return output, nil
}
