...
func TestPutChkSum(t *testing.T) {
	assert := assert.New(t)
	err := PutChecksum("testPutCheckSum", "testthis")
	assert.Nil(err)
...
func TestGetChkSum(t *testing.T) {
	assert := assert.New(t)
	mockFileExists(true)
	_ = PutChecksum("testPutCheckSum", "testthis")
	tstChkSum := GetChecksum("testPutCheckSum")
...
func TestGetChkSumNoFile(t *testing.T) {
	...
	tstChkSum := GetChecksum("chksum-pcidata.txt")
